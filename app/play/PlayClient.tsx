...
/** ===========================
 *  Title sanitizer (display only)
 *  =========================== */
function sanitizeName(name: string) {
  let s = String(name ?? '');
  s = s.replace(/\((?:\s*redone[^)]*|lv\s*\d+[^)]*)\)/gi, '');
  s = s.replace(/(?:^|\s)[-–—]?\s*LV\s*\d+\b/gi, '');
...
function Card({
  ...
  const displayName = sanitizeName(card.name);

  return (
    <div style={styles.card}>
      <div style={styles.cardHead}>
        <strong style={styles.name} title={displayName}>{displayName}</strong>
      ...
      <div style={styles.body}>
        ...
        <div style={{ ...styles.imageBox, width: imgW, height: imgH }}>
          ...
          <CardImage name={displayName} id={card.id} src={card.image!} eager />
        </div>
