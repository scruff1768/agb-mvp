 ...
 * - NEW:     /api/cards?source=local -> load from public/amica_cards.json
 *            or set USE_LOCAL_CARDS=true in env to make local JSON the default
 */
...
function genStatsFor(meta: CanonMeta, idx: number): CardStats {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    const val = Math.max(10, base + noise + ((bump as any)[order[i]] || 0));
    arr.push(val);
    remain -= val;
  }
  let i = 0;
...
/* ---------------- NEW: local JSON loader ---------------- */

function loadLocalJSON(): Card[] {
  const path = join(process.cwd(), "public", "amica_cards.json");
...
export async function GET(req: NextRequest) {
  ...
  const source = (qs.get("source") || "").toLowerCase();
  const useLocal = source === "local" || String(process.env.USE_LOCAL_CARDS).toLowerCase() === "true";

  try {
    let cards: Card[];

    if (useLocal) {
      // NEW: load balanced cards from local JSON
      cards = loadLocalJSON();
    } else {
      // Existing Supabase-aware flow
    ...
    // Default: plain array (back-compat)
